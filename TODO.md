...
- [x] **"프로젝트 선택" 탭에서 AI 모드일 때 대시보드로 자동 이동하지 않는 것처럼 보이는 문제
  (2026-08-08, 사용자 리포트: "규칙 기반은 자동 이동하는데 AI 기반은 클릭해도 반응 없음").**
  코드 분석 결과 `ProjectSelect.vue`의 `onSelect()`는 AI/규칙 기반 분기 없이 항상 순차적으로
  `scan → diagnose → router.push('/')`를 실행하므로, 이동 자체가 안 되는 버그가 아니라 두 가지
  원인의 조합으로 판단: (1) AI 진단은 네트워크 호출이라 몇 초가 걸리는데 그동안 화면에 아무
  피드백이 없어 "반응 없음"으로 보임 (2) `ai-diagnosis.js`의 Anthropic 클라이언트 호출에
  타임아웃이 전혀 설정돼 있지 않아, 네트워크 문제 등으로 응답이 멈추면 규칙 기반 폴백으로
  넘어가지도 못하고 무한정 대기할 수 있었음. **조치**: ① `ProjectSelect.vue`에 `isProcessing`
  로딩 상태 추가 — Dashboard.vue의 `isScanning`/`isDiagnosing` 패턴과 동일하게 처리 중 안내
  문구를 보여주고 카드 영역을 비활성화(중복 클릭 방지), `finally`로 항상 해제. ② 공식 SDK 문서
  (`RequestOptions.timeout`, ms 단위, `create(params, options)`의 두 번째 인자로 전달)를
  확인해 `ai-diagnosis.js`의 `client.messages.create()` 호출에 15초 타임아웃을 추가 — 타임아웃
  발생 시 기존 catch 블록이 그대로 규칙 기반 폴백으로 넘기고 `_aiFallbackReason`에 사유를 실어
  화면에 보여줌(신규 로직 없이 기존 폴백 경로 재사용). 단위 테스트 2개 추가(타임아웃 옵션이
  실제로 API 호출에 실려 가는지 확인 + 느린 응답을 시뮬레이션해 규칙 기반으로 폴백하는지 확인)
  — 총 144개 테스트 통과.

- [x] **진단 카드의 "선택/입력" 액션에 실행 결과 피드백이 전혀 없던 UX 결함 (2026-08-08,
  사용자 리포트: SSH/HTTPS 선택 버튼을 눌러도 반응이 없어 보이고, "자동 복구 계속" 버튼은
  비활성화라 초보 사용자가 어떻게 진행해야 할지 방황함).** 코드 확인 결과 버튼은 실제로
  `set_origin_protocol` 스텝을 즉시 실행하고 재스캔까지 정상 동작했지만(`IssueItem.vue`),
  실행 결과를 전혀 화면에 보여주지 않고 재스캔으로 카드가 통째로 교체돼 "눌러도 아무 일도
  안 일어난 것"처럼 보였음 — 기능은 있으나 피드백이 없는 전형적 UX 결함으로 판단, 근본
  수정 진행. **조치**: ① `IssueItem.vue`의 `applyStep()`이 스텝 실행 결과 메시지를
  `actionSuccess`에 담아 "✅ {메시지} (곧 다시 스캔합니다...)"로 잠깐 보여준 뒤(1.5초)
  재스캔하도록 수정(input/choice 두 액션 타입 모두 공유하는 공통 로직이라 한 번에 적용됨).
  ② `origin-choice.js` 규칙의 설명 문구를 "버튼을 누르면 그 즉시 origin이 바뀌고
  재스캔되며, 아래 '자동 복구 계속'과는 별개로 이 카드에서 바로 실행된다"고 명시. ③
  `ActionBar.vue`에 "자동 복구 계속"이 비활성화된 이유를 설명하는 안내 문구 추가("자동으로
  처리 가능한 항목이 없습니다 — 위 진단 결과의 안내를 먼저 따라 문제를 해결하세요"). 브라우저에서
  가짜 `window.electronAPI`로 스크린샷 속 시나리오(SSH+HTTPS 인증정보 모두 있음)를 그대로
  재현해, 클릭 후 151ms 시점에 성공 메시지가 나타나 1515ms까지 유지되다 재스캔되는 것을
  DOM 폴링으로 직접 확인. 총 144개 테스트 통과(로직 변경 없이 UI 피드백만 추가한 컴포넌트라
  기존 엔진 테스트는 영향 없음).

- [x] **위 성공 메시지의 1.5초 자동 사라짐 → 사용자가 직접 닫는 방식으로 변경 (2026-08-08,
  사용자 요청: "1.5초로는 메시지를 확인할 시간이 부족하다, 종료(✕) 아이콘으로 직접 닫게
  해달라").** `IssueItem.vue`의 `applyStep()`에서 `setTimeout(..., 1500)`으로 자동
  재스캔하던 로직을 제거하고, 성공 메시지 옆에 `✕` 닫기 아이콘(`dismissSuccess()`)을 추가 —
  사용자가 메시지를 다 읽고 직접 닫아야 그 시점에 재스캔한다(그 전까지는 버튼도 비활성화
  상태 유지). `global.css`에 `.action-success`/`.close-icon` 스타일 추가(다른 곳의 다크
  테마 톤과 통일). `origin-choice.js` 설명 문구도 "완료 메시지를 확인한 뒤 닫으면 다시
  스캔합니다"로 갱신. 브라우저에서 3초 대기해도 메시지가 사라지지 않음을 확인한 뒤, ✕ 클릭
  시 즉시 메시지가 닫히고 재스캔되는 것을 직접 확인. 총 144개 테스트 통과.

- [x] **문제 해결 후에도 상단 뱃지/진단 요약이 "달라진 게 없어 보이는" 문제 (2026-08-08,
  사용자 요청: HTTPS/SSH 선택 → 안내 메시지 닫기 후 재스캔되면 상단 뱃지엔 "발견된 문제
  없음", 진단 결과엔 "문제를 해결했습니다"로 명확히 바뀌어야 사용자 친화적이지 않겠냐는
  제안).** 기존엔 문제 0건이 되면 `Dashboard.vue`가 뱃지 텍스트를 아예 빈 문자열로 만들어
  뱃지 자체가 사라졌음 — "방금 문제를 해결했다"는 긍정적 신호가 전혀 안 보여 오히려
  아무것도 안 바뀐 것처럼 느껴졌음. **조치**: ① `Dashboard.vue`에 `badgeText`/`badgeType`
  computed 추가 — 진단 전엔 뱃지 숨김(기존과 동일), 진단 후 0건이면 "발견된 문제 없음"을
  `badge-ok`(초록)로 명시적으로 보여줌(기존엔 숨겨서 "없어짐"과 "아직 진단 안 함"을
  구분할 수 없었음). ② `rule-engine/index.js`의 0건 요약 문구를 "발견된 문제가 없습니다.
  push를 진행하세요."→"발견된 문제를 모두 해결했습니다. push를 진행하세요."로 변경(AI 경로는
  모델이 스스로 문구를 만들므로 별도 수정 불필요 — 뱃지 쪽 fix만으로 소스 무관하게 적용됨).
  `tests/integration/scan-diagnose-recover.test.js`의 해당 문구 검증도 함께 갱신. 브라우저에서
  origin_choice 시나리오를 상태 유지되는 mock으로 재현해 SSH 선택→메시지 닫기→뱃지가
  "문제 1건 발견"(주황)에서 "발견된 문제 없음"(초록)으로, 요약 문구가 실제로 바뀌는 것을
  직접 확인. 총 144개 테스트 통과.

- [x] **바로 위 뱃지 수정을 실제 앱에서 테스트해도 "적용 안 됨"으로 보였던 진짜 원인 —
  캐시 문제가 아니라 `origin_choice` 규칙이 설계상 절대 "해결"되지 않는 로직 결함이었음
  (2026-08-08, 사용자가 서버 재시작·프로세스 강제 종료까지 했는데도 안 바뀐다고 재확인
  요청).** 처음엔 개발 서버/Electron 캐싱 문제로 의심해 프로세스를 전부 정리했지만
  (`vite`/`electron` 중복 프로세스 2세트가 떠 있던 것도 사실이라 정리는 필요했음), 그래도
  안 바뀐다는 재확인을 받고 근본 원인을 다시 추적한 결과 — `detectCorrectOrigin()`
  (`recovery-context.js`)은 SSH 키와 HTTPS 인증정보가 **둘 다** 있으면 `correctOrigin`을
  절대 계산하지 않고(`null`), `origin-choice.js`는 `correctOrigin`이 null이고 두 인증
  수단이 다 있으면 무조건 카드를 띄운다 — **origin 프로토콜을 SSH로 바꾸든 HTTPS로
  바꾸든 두 인증 수단 자체는 그대로 남아있으므로, 이 카드는 사용자가 무엇을 선택해도
  절대 사라지지 않는다(설계상 정상 동작).** 그런데 어제 만든 "문제 0건 → 뱃지 초록/해결
  문구" 로직은 이 카드를 다른 문제와 똑같이 "건수"에 포함시켰기 때문에, SSH+HTTPS를 동시에
  쓰는(스크린샷 속 실제) 시나리오에서는 아무리 선택해도 절대 0건이 될 수 없어 뱃지가 영원히
  안 바뀌었던 것 — 어제 브라우저 검증 때 상태 유지 mock을 "선택하면 사라지는" 걸로
  단순화해서 만든 바람에 이 실제 동작을 못 잡아냈음(반성 — mock을 실제 규칙 로직과
  다르게 단순화하면 이런 걸 놓친다). **조치**: `severity:'info'`는 "고장"이 아니라
  "참고용 선택지"라는 의미 구분을 살려, `rule-engine/index.js`의 `summary`와
  `Dashboard.vue`의 `issueCount`(뱃지) 계산 둘 다 `severity !== 'info'`인 항목만 "문제
  건수"로 센다 — 카드 자체는 `issues`에 그대로 남아 계속 보여주되(원하면 언제든 다른 프로토콜로
  바꿀 수 있게), 건수 집계·뱃지·요약 문구에서는 제외한다. 단위 테스트 1개 추가(origin_choice만
  있는 상태에서 summary가 "문제 없음"으로 뜨는지 확인) — 총 145개 테스트 통과. 브라우저에서
  실제 규칙과 동일하게(선택해도 카드가 안 사라지는) mock을 다시 만들어, 첫 스캔부터 이미
  뱃지가 "발견된 문제 없음"으로 뜨고 SSH 선택 후에도 그대로 유지되는 것을 확인.

- [x] **개발 모드에서 앱 실행 시 개발자 도구 창이 항상 자동으로 함께 뜨는 문제 (2026-08-08,
  사용자 리포트: 스크린샷 첨부, 서버 실행 후 항상 DevTools 창이 등장).** `src/main/index.js`의
  `isDev` 분기에 있던 `mainWindow.webContents.openDevTools()` 호출 때문 — 초기 스캐폴딩 때
  디버깅 편의로 넣어둔 것으로 프로덕션 빌드(`dist/index.html` 로드 경로)에는 영향 없었지만,
  개발 모드에서 매번 자동으로 뜨는 게 거슬린다는 요청에 따라 제거. 필요하면 앱 창에서
  Cmd+Option+I로 언제든 수동으로 열 수 있음. (스크린샷에 함께 보였던 "Electron Security
  Warning: unsafe-eval" 경고는 별개 사항 — Vite dev 서버의 HMR이 쓰는 것으로 dev 전용이고
